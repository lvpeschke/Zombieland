...
define
   ...
   proc {CheckKill X Y F ZombieNumber}
      {System.show 'Zombie : checkkill'}
      local FF FL FR AckF AckL AckR BraveF in
	 FF = [~F.1 ~F.2.1]
	 ...
	 of brave(BraveF NBullets) then
	    ...
	    else {System.show 'Zombie :Face : Le brave est tue'} {Send Config.bravePort kill} end
	 else 
	    skip
	 end
	 case AckL
   ...
   fun {ZombieState ZombieNumber Init}
      ZSid = {Config.newPortObject Init
		 ...
		 [] yourturn then % zombie active
		    ...
		    [] go then
		       local Facing NBullets in
			  
			  {CheckKill Line Col F ZombieNumber}
